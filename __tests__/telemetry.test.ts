...
describe('Telemetry', () => {
  ...
  // FORK DIFF: telemetry defaults to OFF in @stupidloud/codegraph (upstream
  // defaults on). The vast majority of tests below assume telemetry is on so
  // they can exercise recording/sending — inject CODEGRAPH_TELEMETRY=1 here so
  // those tests continue to behave as written. Tests that need to exercise the
  // fork's actual default-off path override with `env: {}`.
  ...
  describe('consent precedence', () => {
    it('defaults to DISABLED when nothing decides otherwise (fork inversion of upstream)', () => {
      const t = make({ env: {} });
