...
- Semantic (vector) search now runs on **Windows**, and uses a real approximate-nearest-neighbor index everywhere instead of scanning every stored vector by hand. The previous vector engine had no Windows build at all and, on the bundled runtime, never actually loaded — so semantic search quietly fell back to a brute-force scan on every platform. It now uses [sqlite-vec](https://github.com/asg017/sqlite-vec), which ships prebuilt for Windows, macOS, and Linux, so results come back faster on large projects and Windows users get semantic search for the first time. Semantic search stays opt-in (it needs an embedding API key); the first `codegraph index` after upgrading re-embeds your symbols once into the new index. On the rare platform sqlite-vec doesn't prebuild for (Windows on ARM), semantic search is simply unavailable rather than failing — the graph and keyword search are unaffected.
